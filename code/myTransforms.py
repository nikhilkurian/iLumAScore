...
class ElasticTransform:
    ...
    def __call__(self, img):
        img = np.array(img)
        shape = img.shape
        dx = self.random_state.rand(*shape) * 2 - 1
        dy = self.random_state.rand(*shape) * 2 - 1
        dz = self.random_state.rand(*shape) * 2 - 1
        dx = gaussian_filter(dx, self.sigma, mode="constant", cval=0) * self.alpha
        dy = gaussian_filter(dy, self.sigma, mode="constant", cval=0) * self.alpha
        dz = gaussian_filter(dz, self.sigma, mode="constant", cval=0) * self.alpha
        ...
        return torch.from_numpy(distored_image)
...
class ToPILImage(object):
    ...
    H x W x C to a PIL Image while preservig the value range.
